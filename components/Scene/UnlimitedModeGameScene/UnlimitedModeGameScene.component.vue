...
export default defineComponent({
  ...
  setup() {
    ...
    const calculateStats = async () => {
      const correctAnswers = await store.getters['unlimited/correctAnswers']
      const wrongAnswers = await store.getters['unlimited/wrongAnswers']
      const passedAnswers = await store.getters['unlimited/passedAnswers']

      window.localStorage.setItem('unlimitedCorrectAnswers', JSON.stringify(correctAnswers))
      window.localStorage.setItem('unlimitedWrongAnswers', JSON.stringify(wrongAnswers))
      window.localStorage.setItem('unlimitedPassedAnswers', JSON.stringify(passedAnswers))
    }
    ...
    watch(
      () => myAnswers.value,
      currentValue => {
        const passedAnswers = store.getters['unlimited/passedAnswers']
    ...
    watch(
      ...
      async value => {
        await store.commit('unlimited/SET_ALPHABET_ACTIVE_INDEX', value)
    ...
    const countdown = computed(() => store.getters['unlimited/countdown'])
    ...
    const pass = () => {
      ...
      window.localStorage.setItem('unlimitedMyAnswers', JSON.stringify(myAnswers.value))
    ...
    const startGame = async () => {
      ...
      setTimeout(() => {
        ...
        window.localStorage.setItem('unlimitedCorrectAnswers', JSON.stringify([]))
        window.localStorage.setItem('unlimitedWrongAnswers', JSON.stringify([]))
        window.localStorage.setItem('unlimitedPassedAnswers', JSON.stringify([]))
        window.localStorage.setItem('unlimitedMyAnswers', JSON.stringify([]))
        window.localStorage.setItem('unlimitedMyAnswers', JSON.stringify([]))
    ...
    const endGame = async () => {
      ...
      store.commit('unlimited/SET_IS_GAME_OVER', {
        isGameOver: true
      })
    ...
    const listenCountdown = async timeData => {
      ...
      await store.commit('unlimited/UPDATE_COUNTDOWN_TIMER', remainTime)
